    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            if (frame_counter < 500):
                ...
                # ear_z = stats.NormalDist(mu=ear_mu, sigma=ear_sigma).zscore(ratio)
                ear_z = (ratio - ear_mu)/(ear_sigma)
                

                mar_mu = stats.mean(mar_arr)
                mar_sigma = stats.stdev(mar_arr)

                # mar_z = stats.NormalDist(mu=mar_mu, sigma=mar_sigma).zscore(MAR)
...
plt.plot(org_ear)
plt.title("EAR Ratio")
plt.xlabel("EAR readings")
plt.show()

plt.plot(org_mar)
plt.title("MAR Ratio")
plt.xlabel("MAR readings")
plt.show()